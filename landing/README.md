...
`hg <verb> <target>` cycles six scenes. **The verb changes rarely and the target rolls
...
launches until `hg` fulfils them (ADR-70). `hg <verb>` for these verbs matches no command yet either.
