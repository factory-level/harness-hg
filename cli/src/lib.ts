...
// HG_CLUSTER_NAME runs a SECOND, isolated local loop beside the default one
// (a proof on a clean cluster while the dogfooding env keeps running):
// pair it with HERMES_GITOPS_HOME. The registry container and the k3d
// network derive from it; ports and forwards are per-state already.
export const CLUSTER_NAME = process.env["HG_CLUSTER_NAME"] || "hermes-gitops-cli";
