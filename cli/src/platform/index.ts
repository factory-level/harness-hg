...
export function applyBundleChildApps(
  ...
  for (const entry of fs.readdirSync(root, { withFileTypes: true })) {
    ...
    const rec = (parseYaml(fs.readFileSync(file, "utf8")) as {
      spec?: { profile?: string; app?: string; application?: string; source?: { repo?: string; path?: string; version?: string } };
    })?.spec;
    if (!rec?.profile || !bundle.profiles.includes(rec.profile)) continue;
    if (!rec.application || !rec.source?.path) continue;
    // `repo: local` means a chart inside the platform repo, which is the
    // only shape a bundled member ships today. An oci:// app would need
    // the chart+version source variant; refusing loudly beats emitting
    // an Application that resolves to nothing.
    if (rec.app && seenApps.has(rec.app)) {
      log(`bundle ${bundle.name}: ${rec.app} already deployed for another member - one release per app`);
      continue;
    }
    if (rec.source.repo !== "local") {
      warn(
        `bundle ${bundle.name}: app ${rec.application} pulls from ${rec.source.repo} - ` +
          "only repo:local apps are deployed for bundled profiles today, so this one is skipped",
      );
      continue;
    }
    const app = {
        ...
        sources: [
          {
            ...
            helm: {
              ...
              valueFiles: [
                "$gitops/bootstrap/values/cluster-values.yaml",
                `$gitops/deployments/apps/${entry.name}/values.yaml`,
              ],
            },
          },
          { repoURL: gitopsUrl, targetRevision: "main", ref: "gitops" },
...
export function bundleRepositories(
  state: HgState,
  bundleName: string,
): { name: string; gitAuthSecretRef?: string }[] {
  void state;
  const file = path.join(STAGING, "gitops", "deployments", "bundles", bundleName, "values.yaml");
  if (!fs.existsSync(file)) return [];
  const doc = parseYaml(fs.readFileSync(file, "utf8")) as
    | { spec?: { repositories?: { name?: string; gitAuthSecretRef?: string }[] } }
    | null;
  return (doc?.spec?.repositories ?? [])
    .filter((r): r is { name: string; gitAuthSecretRef?: string } => Boolean(r?.name));
}

/** Ensure a repository credential Secret EXISTS, without ever
 * overwriting a real one.
 *
 * The placeholder is deliberately useless for a private clone - it
 * carries no key. What it buys is a pod that starts and fails visibly
 * instead of one that sits in `Init:0/1` forever on
 * `MountVolume.SetUp failed ... secret not found`, which is a state
 * nothing surfaces and nobody is watching at 3am.
 *
 * A real credential is somebody's deliberate act and is labelled to
 * survive; this only fills a hole a rebuild left. */
export function ensureRepositoryAuthSecret(namespace: string, ref: string, repoName: string): void {
  ...
  const manifest = sh([
    ...
    "--from-literal=ssh-privatekey=", "--dry-run=client", "-o", "yaml",
  ]);
  kubectl(["apply", "-f", "-"], { input: manifest });
  warn(
    `repository ${repoName}: created an EMPTY ${ref} in ${namespace} so the pod can start. ` +
      "A private clone will fail until a real credential is supplied through the declared " +
      "secret path - a hand-created Secret does not survive a rebuild.",
  );
}
...
export function applicationOf(state: HgState, name: string): { application: string; bundle?: string } {
  const bundle = bundleApplications(state).find((b) => b.profiles.includes(name));
  if (!bundle) return { application: appOf(name) };
  return {
    application: bundleAppOf(bundle.name, bundle.chart === "eve-bundle" ? "eve" : "hermes"),
    bundle: bundle.name,
  };
}
