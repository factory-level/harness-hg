...
export async function cmdNexus(json: boolean, args: string[], opts: NexusOpts): Promise<void> {
  ...
  if (sub === "features") {
    ...
    // Since #854 Nexus runs in the cluster with its state on a PVC, so
    // the host file above is read by nothing while a local loop is up:
    // there the flags live behind the features route, operator role.
    // The file stays the path for a Hermes-hosted Nexus with no loop.
    let live: { base: string; auth: Record<string, string> } | null = null;
    try {
      const st = loadState();
      const port = st.expose?.["nexus"]?.localPort;
      if (port && st.ports?.identity) {
        live = {
          base: `http://127.0.0.1:${port}/api/plugins/hermes-gitops/nexus/features`,
          auth: { authorization: `Bearer ${await tokenFor(st, "operator", "nexus")}` },
        };
      }
    } catch {
      live = null;
    }
    const liveCall = async (method: string, body?: unknown): Promise<Record<string, boolean>> => {
      const r = await fetch(live!.base, {
        method,
        headers: { ...live!.auth, "content-type": "application/json" },
        body: body === undefined ? undefined : JSON.stringify(body),
        signal: AbortSignal.timeout(8000),
      });
      // GET answers the registry list; PUT/DELETE answer the resolved map.
      const doc = (await r.json()) as { detail?: string; features?: { id: string; enabled: boolean }[] | Record<string, boolean> };
      if (!r.ok) throw new CliError(`nexus features: ${method} ${live!.base} -> ${r.status} ${doc.detail ?? ""}`);
      const f = doc.features ?? {};
      return Array.isArray(f)
        ? Object.fromEntries(f.map((x) => [x.id, Boolean(x.enabled)]))
        : Object.fromEntries(Object.entries(f).map(([k, v]) => [k, Boolean(v)]));
    };
    let stored: Record<string, boolean> = {};
    if (live) {
      stored = opts.reset ? await liveCall("DELETE") : await liveCall("GET");
    } else {
    ...
    if (enable.length > 0 || disable.length > 0) {
      ...
      if (live) {
        stored = await liveCall("PUT", { version: 1, enabled: next });
      } else {
    ...
    if (json) {
      jsonOut({ command: "nexus-features", ok: true, registry: regPath, state: live ? live.base : statePath, features: resolved });
      return;
    }
