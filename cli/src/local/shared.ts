...
import { NormalizedConnectionBinding, compileConnections, compileConnectionsFromRoot, connectionFiles, loadConnectionDeclarations, mergeConnectionsIntoBundles, writeConnectionTree } from "../connection/compile.ts";
...
/** The compiled topology tree (deployments/{agents,apps,endpoints,
 * communication,connections}, catalog/) into the working copy - what
 * `hg topology emit` writes, minus its knobs. `up` applies bundle child
 * apps FROM deployments/apps, so the tree it applies from must be the one
 * this checkout compiles to: the previous state was a tree from whatever
 * hand emit last ran, months stale (found live: charts/test-page, a path
 * that no longer existed). Non-fatal like the nexus emit: plan errors keep
 * the previous tree and say so. */
export function emitTopologyInto(state: HgState, work: string): void {
  const root = state.profileDir;
  try {
    ...
    if (!plan.ok) {
      ...
      log(`! topology tree NOT emitted (${n} plan error(s)) - bundle child apps keep their previous records; see \`hg topology doctor\``);
      return;
    }
    ...
    writeTree(path.resolve(work), tree);
    ok("topology tree emitted into the gitops repo (deployments/, catalog/)");
  } catch (err) {
    log(`! topology tree NOT emitted (${err instanceof Error ? err.message : String(err)}) - fix with \`hg topology emit\``);
  }
}

export function emitNexusInto(state: HgState): (work: string) => void {
  return (work) => {
    emitTopologyInto(state, work);
