...
import { appOf, bundleAppOf, loadState, loadTestConfig, log, ok, profileCtxs, saveState } from "../lib.ts";
import { appStatus, appStatusOf, applyApplication, applyBundleApplication, applyBundleChildApps, bundleApplications, bundleProfileSecretRefs, bundleRepositories, ensureAppProject, ensureArgoCd, ensureCluster, ensureEnvSecret, ensureEso, ensureEveRuntimeImage, ensureEventRouter, ensureExposures, ensureFleetDashboard, ensureGitAuthSecret, ensureIdentity, ensureMonitoringPair, ensureNexus, ensureOciRepoSecrets, ensurePlatformExposures, ensurePlatformMirror, ensureRepositoryAuthSecret, ensureServers, ensureTools, gatewayIp, importAgentImage, publishGitops, retirePerProfileApplication, syncProfile, waitFor, ensureLoki, ensureWiki } from "../platform/index.ts";
...
export async function cmdUp(): Promise<void> {
  ...
  for (const b of bundles) {
    ...
    for (const repo of bundleRepositories(state, b.name)) {
      ...
      ensureRepositoryAuthSecret(b.namespace, repo.gitAuthSecretRef, repo.name);
    }
  }
