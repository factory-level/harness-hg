...
import { CliError, HgState, KCTX, ProfileCtx, appOf, freePort, jsonOut, kubectl, loadState, loadTestConfig, log, nsOf, ok, profileCtxs, repoScriptsAllowed, sh } from "../lib.ts";
...
export async function tierRegister(ctx: ProfileCtx, state: HgState): Promise<boolean> {
  ...
  const { application, bundle } = applicationOf(state, ctx.name);
  ...
  for (const check of testCfg.smoke) {
    ...
    const pf = Bun.spawn(
      ["kubectl", "--context", KCTX, "-n", ns, "port-forward",
        `svc/${check.service}`, `${local}:${check.port}`],
