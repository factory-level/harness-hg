...
| The communication plane | `cli/src/topology/{communication,envelope}.ts`, `cli/src/communication/` (incl. its `prove.ts`), `control-plane/event-router/chart/` | `bun test` + `make chart-test` golden + `hg communication prove` (cluster) | **Solid locally** — compiles, emits, routes, queues (FIFO/DLQ/replay), gates external webhooks, delivers to Discord; cloud providers + Slack/Teams deferred (ADR-39) |
...
| Chat conversations (ADR-154) | `examples/eve-agent/agents/echo/agent/channels/discord.ts`, `_docs/wiki/platform/chat-conversations.md` | `hg agent prove` **EVE024** (the gateway session is live and there is exactly ONE); the conversation itself proven by hand against a real workspace | **Built locally** — mention → thread → in-thread follow-ups on Discord, dispatched by eve's Chat SDK `send()`. The listener is ours because both of the adapter's own modes fail (see ADR-154); Slack/Teams/Telegram are native and unbuilt here |
