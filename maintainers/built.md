...
| The destination-host reconciler | `cli/src/reconcile/`, `infra/src/components/reconciler` | `cd cli && bun test tests/reconcile.test.ts` (command and team kinds against a real bare repository; pending backoff; v1alpha2 record vs schema; `EnvironmentFile`), `cd infra && bun test tests/config.test.ts`, `make schema-validate` (reconciliation-status v1alpha2 fixtures), `plugin/tests/test_nexus_plugin_api.py` | **Partial** — pulumi apply + https fetch unexercised (ADR-54 Cost); the team kind (ADR 0191) has run against a stubbed resume only, never a live installation |
...
| Tracked workspaces (ADR 0197) | `agent-bundle-contracts/environment-workspaces/v1alpha2/`, `cli/src/workspace/{bindings,index,command}.ts`, `cli/src/harness/manifest.ts`, `harness/eve/charts/eve-agent/{files/workspace-sync.sh,files/workspace-metrics.mjs,templates/workspace-alerts.yaml}`, the shared `files/boot.sh` | `make schema-validate` (the v1alpha2 fixtures, six tracked negatives); `cd cli && bun test tests/workspace-bindings.test.ts tests/workspace.test.ts tests/harness-manifest.test.ts` (compile, refusals, emission bytes, verify freshness, manifest channel); `make chart-test` (the `eve-workspace-tracked` golden and structural assertions, guard negatives, the `<namespace>-alerts` contact-point default and its override, the bundle refusal, and `infra/scripts/workspace-sync-test.sh`: the rendered refresh against local git) | **Built locally** — contract, compiler, verify, chart and refresh proven offline; never run in a cluster, never scraped, `WorkspaceStale` never evaluated by Grafana; bundled agents refuse it; `hg team status` and Nexus show no freshness; no persona adopts it yet |
| The eve chart | `harness/eve/charts/{eve-agent,eve-bundle}/`, `harness/eve/image/` | `make chart-test` goldens `eve-*`, `bundle-eve-*` + guards and negatives; `infra/scripts/overlay-apply.test.mjs` + `infra/scripts/overlay-boot-test.sh` (operator overlay merge and the rendered boot path, run by render-test); the `eve-workspace-tracked` golden, its structural assertions, six tracked-guard negatives, the bundle refusal and `infra/scripts/workspace-sync-test.sh` (tracked workspaces, ADR 0197: the rendered refresh against local git, run by render-test); `make eve-boot-test` (Docker: image, boot convergence, route contract, credential rotation); `cli/e2e-local.sh` steps 1–14 on a real Eve agent — **which cannot run from a git worktree**: the wiki image derives its version from git history and a linked worktree's `.git` file resolves to nothing inside the build container, so `make e2e` runs from the main checkout or a clone (`control-plane/wiki/image/build.sh` now refuses early and says so) — (the agent's receipt, startup gate and dev-loop rebuild; `hg team status` proving it, catching drift from its lock and following a rollback; a team-kind watcher, its locked platform revision as a real worktree, the published v1alpha2 status and live `hg team status`, all against a k3d cluster with Argo CD); `cli/tests/versions.test.ts` pin parity and allowed-runtime shape (ADR 0192); the startup-gate render assertions (version metadata on the workload but never a selector, label length, probe kinds, the byte-identical `verify-build.mjs`) and `make eve-boot-test`'s receipt and gate legs (ADR 0195); `cli/tests/backup.test.ts` Eve contract block; the cloudflare/ingress **byte-equality** assertion in `render-test.sh` (cloudflare must add nothing per-instance) | **Partial** — the agent runtime (ADR-149) with its backup routine, child Applications, workspace bindings and the bundle chart (ADR-150); durable state relocated out of the checkout; proven on the local loop including a restore round-trip, not on a live Argo ApplicationSet or factory-server; `justbash()` only. Reaches the Cloudflare edge over the shared control-plane tunnel (ADR-156), with no per-instance tunnel and no author-declared exposure. **`make eve-boot-test` is not wired into CI** (`ci` is disabled repo-wide, `.github/workflows/DISABLED.md`): a regression in the pod's boot or route contract is caught by a developer running the target locally, by nothing else |
| The eve emitter | `plugin/gitops_emitter/harness/eve.py`, `plugin/gitops_emitter/emit_cli.py`, `agent-bundle-contracts/eveagent/v1alpha3/` | `make pytest` (`test_eve.py` incl. `TestOverlays`, `test_emit_cli.py` against a local bare repo); `make schema-validate` fixtures (v1alpha3 overlay refusals) | **Solid** — push-driven; resolves `apps` through the Hermes pipeline's `resolve_apps` (`--app-values`), copies `backup`, carries approved operator overlays from `--overlays-file` in application order (unique ids, one writer per target); publishes through the same `publish_record` as the Hermes hook |
...
## Team installation coordinator

| Unit | Directory | Gate | Status |
|---|---|---|---|
| Approved external skill manifests and offline packages | `cli/src/skills`, `plugin/gitops_emitter/skills.py` | `bun test cli/tests/skills.test.ts`; `pytest plugin/tests/test_skills.py` | Partial |

Local Git, hash and recovery tests cover the installer. Live Eve discovery of shared plugin
entrypoints and deployed acceptance require a production run; fixtures do not prove them.

| Unit | Directory | Gate | Status |
|---|---|---|---|
| Two-phase source/bootstrap skills and complete publisher | `skills/hg-team-{onboard,bootstrap}`, `cli/src/team` | `cd cli && bun test tests/team.test.ts tests/team-credentials.test.ts tests/team-inputs.test.ts` (content-only commits skip, ADR 0198); `cd infra && bun test tests/eve-agent.test.ts` (credential delivery parity); CLI typecheck; skill validator | Partial |

The local gate covers additive multi-team projection, ownership/adoption, pending PR resume,
merge retry, secret scrubbing, stage invalidation, bounded recovery and the credential gate (ADR 0196). Live provider,
production-container and deployed acceptance paths are implemented but not validated by fixtures.

Named watcher readback is covered by `plugin/tests/test_nexus_plugin_api.py`: a failed or stale
named source cannot be hidden by a healthy default watcher. The chart grants namespace-scoped
ConfigMap list access for discovery. No Secret-list permission is added.
