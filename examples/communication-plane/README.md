...
- **platform-sre** — subscribes: `observability.alert/v1` from its monitoring
  ...
  (`company_discord#channel-1`, `#channel-2`). Also declares one external
...
- `environment/communication.yaml` — the local default: `company_discord`
  rebound to the **recording** provider (messages are captured, never sent),
  durable transport `redis-streams`.
- `environments/discord-sandbox/` — the same routes with `company_discord`
  bound to the **real Discord plugin**; the bot token comes from the
  `HG_DISCORD_BOT_TOKEN` env var (a reference — never committed).
...
hg chatops tail company_discord#channel-1 --json
